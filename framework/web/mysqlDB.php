...
class mysqlDB {
    ...
    public static function getInstance($db_name=""){
        ...
        if(self::$mysqlDB==null||!isset(self::$mysqlDB[$db_name])){
            ...
            $conf = self::$db_config[$db_name][$config_key];
            if($db_name!==$conf['db_name']) throw new Exception("db config error,DB:$db_name key:$config_key db_name must equal to '$db_name';");
            self::$mysqlDB[$db_name] = new mysqlDB($conf);
        }else{
